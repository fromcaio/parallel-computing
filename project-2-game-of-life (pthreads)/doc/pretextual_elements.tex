...
\begin{resumo}[Abstract]
    \begin{otherlanguage*}{english}
        ...
        This project implements Conway's Game of Life in C with two versions: a sequential baseline and a shared-memory parallel variant using POSIX threads. A row-block decomposition assigns contiguous slices of the grid to each thread, and per-generation barriers maintain correctness while swapping buffers. The codebase separates the update kernel from orchestration and I/O, enabling timing that excludes file writing. We measured execution time, speedup, efficiency and peak memory usage across grid sizes up to $5000 \times 5000$ and thread counts ranging from 1 to 40. The results indicate strong scalability within physical core limits.
